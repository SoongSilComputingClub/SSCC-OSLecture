...
void s1main(void)
{
    ...
    for (current_lba = rootdir_lba; current_lba < cluster_start_lba; ++current_lba) {
        int i;
...
file_found:
    if (!entry || !entry->file.name[0]) {
        PRINT_STR("[stage1] VELLUM.X86 not found\r\n");
        return;
    }

    current_cluster = entry->file.cluster_location;
    load_dest = (uint32_t *)__stage2_start;
    ...
    while (current_cluster <= FAT12_MAX_CLUSTER) {
        ...
        for (i = 0; i < bytes_per_cluster / (int)sizeof(uint32_t); ++i) {
