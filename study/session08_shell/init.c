...
static ssize_t bios_stdout_write(void *cookie, const char *buf, size_t count)
{
    (void)cookie;
...
static const struct cookie_io_functions bios_stdout_io = {
    .write = bios_stdout_write,
};
...
__noreturn void _pc_init(void)
{
    ...
    freopencookie(NULL, "w", bios_stdout_io, stdout);
    freopencookie(NULL, "w", bios_stdout_io, stderr);

    _pc_gdt_init();
    VlIntP_Init();
    _pc_pic_remap_int(0x20, 0x28);

    fprintf(stderr, "session08: shell parser and expansion\n");
