...
)
{
    /*
     * TODO(session 09-1):
     * Query VBE mode information and translate it into `video_hw_mode_info`.
     */
    (void)dev;
    (void)mode;
    (void)hwmode;
...
static status_t fb_invalidate_vbe(struct device *dev, int x0, int y0, int x1, int y1)
{
    /*
     * TODO(session 09-2):
     * Mark every diff region touched by the rectangle `[x0, y0] - [x1, y1]`.
     */
    (void)dev;
    (void)x0;
    (void)y0;
...
static status_t fb_flush_vbe(struct device *dev)
{
    /*
     * TODO(session 09-3):
     * Walk modified diff regions and copy the software framebuffer into the
     * hardware framebuffer using `convert_color`.
     */
    (void)dev;
    return STATUS_NOT_IMPLEMENTED;
}
